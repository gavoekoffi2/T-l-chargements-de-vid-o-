...
def ov_intro(img, t, dur):
    ...
    # barre accent qui grandit
    bw=int(lerp(0, 620, eo(t/0.6)))
    rrect(d,(W//2-bw//2, 250, W//2+bw//2, 258), 4, col(GOLD, A))
    # titre 2 lignes
    glow_text(img, W//2, 280, "IMMIGRANTS", font(96), col(WHITE,A), CYAN, ga=int(A*0.4))
    glow_text(img, W//2, 390, "FRANCOPHONES", font(96), col(GOLD,A), GOLD, ga=int(A*0.4))
    sub_a=life(t,dur)*clamp01((t-0.4)/0.4)*255
    ctext(d, W//2, 520, "•  AU  CANADA  •", font(40), col(CYAN,sub_a))

def ov_canada(img, t, dur):
    A=life(t,dur,0.5,0.5)*255
    d=ImageDraw.Draw(img)
    panel_glass(img,(70,180,W-70,640),28,alpha=int(0.92*A),border=CYAN,border_a=int(0.7*A))
    d=ImageDraw.Draw(img)
    ctext(d, W//2, 215, "PARTOUT  AU  CANADA", font(44), col(CYAN,A))
    provinces=["QUÉBEC","ONTARIO","ALBERTA","SASKATCHEWAN",
               "MONTRÉAL","OTTAWA","VANCOUVER","NOUVEAU-BRUNSWICK"]
    # chaque province s'allume successivement
    x0,y0=130,300; cw=(W-260)//2; rh=78
...
def ov_counter(img, t, dur):
    A=life(t,dur)*255
    d=ImageDraw.Draw(img)
    panel_glass(img,(120,300,W-120,720),30,alpha=int(0.93*A),border=RED,border_a=int(0.6*A))
    d=ImageDraw.Draw(img)
    ctext(d, W//2, 340, "LE  PIÈGE", font(40), col(RED,A))
    # compteur jobs 1->3
    jobs = 1 + int(clamp01((t-0.3)/1.2)*2 + 0.5)
    jobs=min(3,jobs)
    glow_text(img, W//2-180, 410, f"{jobs}", font(180), col(WHITE,A), RED, ga=int(A*0.4))
    ctext(d, W//2-180, 600, "JOBS", font(48), col(DIM,A))
    ctext(d, W//2+30, 470, "→", font(120), col(GOLD,A))
    # heures 0 -> 80
    hrs=int(clamp01((t-0.6)/1.4)*80)
    glow_text(img, W//2+230, 410, f"{hrs}", font(150), col(GOLD,A), GOLD, ga=int(A*0.4))
    ctext(d, W//2+230, 600, "H / SEMAINE", font(40), col(DIM,A))

def ov_t4(img, t, dur):
    A=life(t,dur)*255
    d=ImageDraw.Draw(img)
    panel_glass(img,(110,280,W-110,720),30,alpha=int(0.93*A),border=RED,border_a=int(0.6*A))
    d=ImageDraw.Draw(img)
    ctext(d, W//2, 320, "TON  SALAIRE  ( T4 )", font(44), col(WHITE,A))
    # barre salaire avec part prélevée
    bx,by,bw,bh=170,430,W-340,90
    rrect(d,(bx,by,bx+bw,by+bh),16,col((40,50,75),int(A)))
    take=clamp01((t-0.5)/1.3)          # part prélevée grandit
    cut=int(bw*0.45*take)
    rrect(d,(bx,by,bx+cut,by+bh),16,col(RED,int(A)))   # prélèvement
    ...
    ctext(d, W//2, 600, "PRÉLEVÉ  CHAQUE  MOIS", font(36), col(DIM,A))
...
def ov_sixans(img, t, dur):
    ...
    glow_text(img, W//2, int(330-(sc-1)*60), "6", font(int(300*sc)), col(GOLD,A), GOLD, ga=int(A*0.5))
    ctext(d, W//2, 660, "ANS  DE  LIBERTÉ", font(64), col(WHITE,A))
    line=int(lerp(0,520,eo(clamp01((t-0.4)/0.6))))
    rrect(d,(W//2-line//2,740,W//2+line//2,746),3,col(CYAN,A))

def ov_liberte(img, t, dur):
    A=life(t,dur,0.5,0.5)*255
    d=ImageDraw.Draw(img)
    panel_glass(img,(90,230,W-90,720),30,alpha=int(0.92*A),border=GREEN,border_a=int(0.6*A))
    d=ImageDraw.Draw(img)
    glow_text(img, W//2, 270, "LIBERTÉ", font(90), col(GREEN,A), GREEN, ga=int(A*0.4))
    ...
    for i,it in enumerate(items):
        ...
        d.text((x, 420+i*72), it, font=font(44), fill=col(WHITE,a))

def ov_cta(img, t, dur):
    A=life(t,dur)*255
    d=ImageDraw.Draw(img)
    panel_glass(img,(90,300,W-90,680),34,alpha=int(0.95*A),border=GOLD,border_a=int(0.8*A))
    d=ImageDraw.Draw(img)
    ctext(d, W//2, 345, "ÉCRIS  EN  COMMENTAIRE", font(42), col(WHITE,A))
    # zone de saisie qui "tape" le texte
    bx,by,bw,bh=160,430,W-320,120
    ...
    # flèche commentaire
    ctext(d, W//2, 600, "envoie-moi  «  ABRAHAM VALEUR  »", font(34), col(CYAN,A))

def ov_endcard(img, t, dur):
    A=life(t,dur,0.5,0.6)*255
    d=ImageDraw.Draw(img)
    panel_glass(img,(70,260,W-70,720),30,alpha=int(0.95*A),border=CYAN,border_a=int(0.7*A))
    d=ImageDraw.Draw(img)
    bw=int(lerp(0,560,eo(t/0.6)))
    rrect(d,(W//2-bw//2,310,W//2+bw//2,318),4,col(GOLD,A))
    glow_text(img, W//2, 345, "ABRAHAM KOFFI AKPOBI", font(58), col(WHITE,A), CYAN, ga=int(A*0.3))
    ctext(d, W//2, 440, "Conseiller en sécurité financière", font(40), col(GOLD,A))
    s2=clamp01((t-0.5)/0.5)*A
    ctext(d, W//2, 520, "Directeur — Finup  ×  Great Way Financial", font(31), col(DIM,s2))
    ctext(d, W//2, 600, "Objectif : 800 conseillers formés d'ici 2030", font(30), col(CYAN,s2))
