...
import overlays from '../overlays2.json';
...
export const LogoOverlayTrack2: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ pointerEvents: 'none' }}>
      {(overlays as Overlay[]).map((o) => {
        ...
        return <LogoCard key={o.id} overlay={o} t={t} fps={fps} />;
      })}
    </AbsoluteFill>
  );
};

const LogoCard: React.FC<{ overlay: Overlay; t: number; fps: number }> = ({ overlay, t, fps }) => {
  ...
  const opacity = interpolate(inSpr, [0, 0.7], [0, 1], { extrapolateRight: 'clamp' }) * outFade;
    ...
    <div style={{
      ...
      <div style={{
        ...
        {glitchPhase > 0 && (
          <>
            <div style={{
              ...
              opacity: glitchPhase * 0.6, mixBlendMode: 'screen',
            }} />
            <div style={{
              position: 'absolute', inset: 0, borderRadius: 28,
        ...
        <div style={{
          ...
          padding: 12, boxShadow: `0 0 24px ${overlay.color}aa`,
        }}>
          <Img src={staticFile(`logos/${overlay.logo}`)} style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          <span style={{
            ...
            textShadow: `0 0 12px ${overlay.color}`, letterSpacing: -1,
          }}>
            {overlay.brand}
          </span>
          {overlay.subtitle && (
            <span style={{
              fontFamily: 'Inter, system-ui, sans-serif',
              fontWeight: 700, fontSize: 24, color: overlay.color,
